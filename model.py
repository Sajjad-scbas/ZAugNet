...
class ZAugGenerator(nn.Module):
    ...
    def set_multiple_gpus(self):
        self.block0 = nn.DataParallel(self.block0, device_ids=opt.device_ids).cuda()
        self.block0.to(f"cuda:{opt.device_ids[0]}")
        self.block1 = nn.DataParallel(self.block1, device_ids=opt.device_ids).cuda()
        self.block1.to(f"cuda:{opt.device_ids[0]}")
        self.block2 = nn.DataParallel(self.block2, device_ids=opt.device_ids).cuda()
        self.block2.to(f"cuda:{opt.device_ids[0]}")
        self.block_tea = nn.DataParallel(self.block_tea, device_ids=opt.device_ids).cuda()
        self.block_tea.to(f"cuda:{opt.device_ids[0]}")
        self.contextnet = nn.DataParallel(self.contextnet, device_ids=opt.device_ids).cuda()
        self.contextnet.to(f"cuda:{opt.device_ids[0]}")
        self.unet = nn.DataParallel(self.unet, device_ids=opt.device_ids).cuda()
        self.unet.to(f"cuda:{opt.device_ids[0]}")
...
class ZAugDiscriminator(nn.Module):
    ...
    def __init__(self, model_name='zaugnet'):
