...
def predict(cfg, dataset, factor, DPM=None):
    ...
    if DPM == 0.5:
        ...
        factor = 2
        DPMS = torch.Tensor([DPM]).to(device)
    ...
    for filename in tqdm(list_of_files, desc='Files'):
        ...
        prediction = torch.zeros((t, int((z-1)*factor)+1, 1, h, w), dtype=torch.uint8, device=device)
        ...
        for dt in tqdm(range(t), desc='Time Range'):
            for dz in range(z-1):
                with torch.no_grad():
                    ...
                    prediction[dt, factor * dz + 0] = torch.clip(image0[0, 0, None, :, :], cfg.min_max[0], cfg.min_max[1]).type(prediction.dtype)
                    prediction[dt, factor*(dz + 1)] = torch.clip(image2[0, 0, None, :, :], cfg.min_max[0], cfg.min_max[1]).type(prediction.dtype)
                    prediction[dt, factor*dz+1 : factor*(dz + 1)] = torch.clip(pred, cfg.min_max[0], cfg.min_max[1]).type(prediction.dtype)

    
        # Convert the prediction tensor to a NumPy array and save it
        prediction = prediction.detach().cpu().numpy()
        tif_path = './predictions/' + str(filename[:-4]) +'-z' + str(int((z-1)*factor)+1) + '_' + model_name +'.tif'
        tif.imwrite(tif_path, prediction, imagej=True)
