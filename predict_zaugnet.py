...
from zaugnet.model import ZAugGenerator
        ...
        for dt in tqdm(range(t)):
            for dz in range(z-1):
                with torch.no_grad():
                    ...
                    prediction[dt, 2 * dz + 0] = torch.clip(image0[0, 0, None, :, :], opt.min_max[0], opt.min_max[1]).type(prediction.dtype)
                    prediction[dt, 2 * dz + 2] = torch.clip(image2[0, 0, None, :, :], opt.min_max[0], opt.min_max[1]).type(prediction.dtype)
                    prediction[dt, 2 * dz + 1] = torch.clip(pred[0, 0, None, :, :], opt.min_max[0], opt.min_max[1]).type(prediction.dtype)
